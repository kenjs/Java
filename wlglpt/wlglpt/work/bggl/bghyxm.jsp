...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#addBtn").click(function(){
				var url = jcontextPath+"/bggl/bghyxm!initMx.action";
    ...
    function onUpdate(hyxmDjxh){
    	var url = jcontextPath+"/bggl/bghyxm!initMx.action?domain.hyxmDjxh="+hyxmDjxh;
		window.showModalDialog(url,window,"dialogHeight:420px;dialogWidth:760px;center:yes;resizable:no;status:no;scroll:yes;help:no;minimize:yes;")
		onRefresh();
    }
    
    var keyValue = "";
	function onDelete( hyxmDjxh){
		keyValue = hyxmDjxh;
		showConfirm("确定要删除么？","yesCallBack");
	}	
	function yesCallBack(){
		 var jsonObj = {"domain.hyxmDjxh":keyValue};
		 var url = jcontextPath+"/bggl/bghyxm!delete";   
	...
	function onRefresh(){
		...
		//请求表格数据
		var url = jcontextPath+"/bggl/bghyxm!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.zt":encodeURI(zt),"domain.rqQ":rqQ,"domain.rqZ":rqZ}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    sortname: 'HYXM_DJXH',				//默认排序 可以是列名称或者是一个数字，这个参数会被提交到后台
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/bggl/bghyxm!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var val = jQuery("#dataList").jqGrid('getCell', cl,"hyxmDjxh"); 	  //获取当前单元格里面的登记序号 
     ...
     /**************************************分页结束****************************************/
    function onQurtyMx(hyxmDjxh){
    	var url = jcontextPath+"/bggl/bghyxm!queryMx.action?domain.hyxmDjxh="+hyxmDjxh;
		window.showModalDialog(url,window,"dialogHeight:420px;dialogWidth:760px;center:yes;resizable:no;status:no;scroll:yes;help:no;minimize:yes;")
		onRefresh();
    }
</script>
...
<body>
<s:form action="bghyxm!query" namespace="/bggl" method="post" id="mainForm" name="mainForm">
