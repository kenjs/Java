...
<script type="text/javascript">
    ...
    function onUpdate(pcDjxh){
    	var url = jcontextPath+"/hypcdhaj!initMx.action?domain.pcDjxh="+pcDjxh;
    	window.showModalDialog(url,window,"dialogHeight:800px;dialogWidth:820px;center:yes;resizable:no;status:no;scroll:yes;help:no;minimize:yes;")
    	onRefresh();
    }
	...
	function onRefresh(){
		...
		//请求表格数据
		var url = jcontextPath+"/hygl/hypcdhaj!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.pcbm4Query":sj,"domain.bz":bz, "domain.pcrCzyDjxh":djrCzyDjxh4Query,"domain.fhrDjxh":fhrDjxh,
		 			"domain.clhm4Query":encodeURI(clhm), "domain.pcdh4Qyuery":encodeURI(pcdh),"domain.sjxm4Query":encodeURI(sjxm),
		 			"domain.pcrqQ":rqq,"domain.pcrqZ":rqz,"domain.fhrMc":encodeURI(fhrMc)}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '状态','安检次数','派车单号','派车日期','车主姓名','车辆号码',
				     '挂车号码','司机姓名','手机号码','其它联系电话','派车人','所属单位','所属部门',''],			 //name 列显示的名称
		     //name 对应结果集里面的属性名称  index 传到服务器端用来排序用的列名称 width 列宽度；align 对齐方式；sortable   是否可以排序
		    colModel :[ 
			  {name:'hstoperationcol', index:'', sortable:false, width:'50', align:'center'},
			  {name:'zt', index:'zt', width:'60', align:'center'}, 
		      {name:'ajcs', index:'ajcs', width:'55', align:'center'}, 
		      ...
		      {name:'gchm', index:'gchm', width:'80', align:'center'}, 
		      {name:'sjXm', index:'sjXm', width:'80', align:'center'}, 
		      {name:'sjSjHm', index:'sjSjHm', width:'100', align:'center'}, 
		      {name:'qtLxDh', index:'qtLxDh', width:'100', align:'center'}, 

		      {name:'pcrCzyMc', index:'pcrCzyMc', width:'80', align:'center'}, 
		      {name:'sjMc', index:'sjMc', width:'90', align:'center'}, 
		      {name:'bmMc', index:'bmMc', width:'90', align:'center'}, 
		      {name:'pcDjxh', index:'pcDjxh', width:'0', align:'center',hidden:true}
		    ],
		    pager: '#pager',					//定义翻页用的导航栏，必须是有效的html元素。翻页工具栏可以放置在html页面任意位置
		    rowNum: <%=rowNum%>,							//在grid上显示记录条数，这个参数是要被传递到后台
		    rowList:[20,50,100,300,500],		//一个下拉选择框，用来改变显示记录数，当选择时会覆盖rowNum参数传递到后台
		    sortname: 'AJ_DJXH',				//默认排序 可以是列名称或者是一个数字，这个参数会被提交到后台
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/hygl/hypcdhaj!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var zt = jQuery("#dataList").jqGrid('getCell', cl,"zt"); //获取当前单元格里面的登记序号 
                var link = "<a href=\"javascript:onUpdate('"+val+"')\"><font color=\"blue\">登记</font></a>";
               var tag='';
                if(zt=='未安检'){
                	tag="<font color=\"red\">未安检</font>"
                }
                else{
                	tag="<font color=\"black\">已安检</font>"
                }
                $("#dataList").jqGrid('setRowData', cl, { 'pcdh': strPc }); 
                $("#dataList").jqGrid('setRowData', cl, { 'zt': tag }); 
...
<body>
<s:form action="hypcdhaj!query" namespace="/hygl" method="post" id="mainForm" name="mainForm">
