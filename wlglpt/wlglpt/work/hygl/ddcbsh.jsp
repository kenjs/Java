...
function onViewPc(pcDjxh){
	...
	function onRefresh(){
		...
		var url = jcontextPath+"/hygl/ddcbsh!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.shbz":shbz,"domain.rqQ":rqQ,"domain.rqZ":rqZ}		//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['序号','pcDjxh','文书审批序号','审批序号','节点序号','<input title="选中/取消选中" type="checkbox" onclick="checkAllJGridChk(event,this,\'xhs\');" />',
			    		'操作','标志','发送人','发送日期','收入','运费','预付','类别','审批人','审批日期','审批结果','派车单号','车辆号码','挂车号码','司机姓名',
			    		'订单编号','客户名称','ddDjxh','货物名称','始发地','目的地','数量','重量','体积',
			    		'处理方式','转入部门','合计','现付','月结','到付','信息费',
			    		'发货人地址','收货人地址',
			    		'派车人','派车日期','派车部门','所属机构'
			    		],			 //name 列显示的名称
