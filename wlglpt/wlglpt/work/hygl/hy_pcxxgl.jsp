...
<script type="text/javascript">
    ...
    function onUpdate(pcDjxh){
    	var url = jcontextPath+"/hygl/hypcxxgl!initMx.action?domain.pcDjxh="+pcDjxh+"&num="+Math.random();
    	navigateMenu(url,'派车信息修改','true');
    	//window.showModalDialog(url,window,"dialogHeight:600px;dialogWidth:870px;center:yes;resizable:no;status:no;scroll:no;help:no;maximize:yes;minimize:yes;")
    	//onRefresh();
    }
    
    var keyValue = "";
	function onDelete(pcDjxh){
		keyValue = pcDjxh;
		showConfirm("确定要删除么？","yesCallBack");
	}	
	function yesCallBack(){
		 var jsonObj = {"domain.pcDjxh":keyValue};
		 var url = jcontextPath+"/hygl/hypcxxgl!delete";   
	...
	function onRefresh(){
		...
		//请求表格数据
		var url = jcontextPath+"/hygl/hypcxxgl!query.action";   
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    colNames:['序号','<input title="选中/取消选中" type="checkbox" onclick="checkAllJGridChk(event,this,\'xhs\');" />',
		              '操作', '派车登记序号','文书审批序号','审批','审批状态代码','派车单号', '派车日期','类别', '装车', '车辆号码', '挂车号码', 
		              '司机', '运费/预付', '收入/到付','重量','体积',
		              '订单编号','客户名称','ddDjxh', '货物名称','送货', '始发地', '目的地', '结算数量', '数量', 
		              '重量', '体积','包装','处理方式','下游单位','合计','现付','月结','到付','信息费','发货地址', '要求发货日期', '收货人','收货地址',
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/hygl/hypcxxgl!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
				...
				var link = "<a href=\"javascript:onUpdate('"+pcDjxh+"')\"><font color=\"blue\">修改</font></a>"
						...
						+"<br><a href=\"javascript:onQingdan('"+pcDjxh+"')\"><font color=\"blue\">清单</font></a>";
