...
<script type="text/javascript">
	...
	function onRefresh(){
		...
		var url = jcontextPath+"/zygl/qykhhwxh!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.ssJgbm":ssJgbm,"domain.djJgbm":djJgbm,"domain.khDjxh":khDjxh,"domain.hwDjxh":hwDjxh}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '物流登记序号', '货物登记序号','客户名称','货物名称','型号名称','型号简称','型号全称',
				     '拼音全称','拼音简称','长度计量单位','长度','宽度',
				     '高度','数量计量单位','重量计量单位','体积计量单位','包装计量单位','结算计量单位',
				     '备注','登记部门','登记人','登记日期',
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var val = jQuery("#dataList").jqGrid('getCell', cl,"hwxhDjxh"); 	  //获取当前单元格里面的登记序号 
