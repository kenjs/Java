...
<script type="text/javascript">
	$(function(){
		...
		$("#delBtn").click(function(){
			...
			if (xhs.length <= 0) {
				showAlert("请先选择需要删除的司机信息！");
				return;
			}
			
			showConfirm("确定要删除选中的司机信息？", "deleteSj");
		});
		...
		$("#saveBtn").click(function(){
			...
			var url = jcontextPath+"/zygl/qyylclxxshcl!save";  
	    	var jsonObj = {"domain.czXm":czXm,"domain.clhm":clhm,"domain.czZjlxDm":czZjlxDm,"domain.czZjhm":czZjhm,
                           ...
                           "domain.clxhwhDjxh":clxhwhDjxh,"domain.gcbz":"N"};   			
	...
	function YesSave(obj){
	    ...
	    $("#mainForm_domain_clDjxh").val(obj.domain.clDjxh);		
		$("#mainForm_zt").val(1);
	...
	function deleteSj(){
		...
		if (xhs.length > 0) {
			...
			jsonStr += jQuery.param(jsonObj);
			var url = jcontextPath+"/zygl/qyylclxxshcl!deleteSjxx";  
			ajaxCommon(url,encodeURI(jsonStr),"doDelSjSuc", false);
		}
	}
	function doDelSjSuc(){ 
        //hideMessage();
	...
	function checkdata(){
		...
		var compareValueArray = [100,20,50,50,
								 20,20,10,10,
								 10,200,
								 100,30,30,50,50];
...
<body>
<%try{ %>
<s:form action="qyylclxxShcl!initMx" namespace="/zygl" method="post" id="mainForm" name="mainForm">
