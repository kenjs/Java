...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#addBtn").click(function(){
				...
				var url = jcontextPath+"/zygl/qythshdz!initMx.action?domain.ssJgbm="+ssJgbm;
    ...
    function onUpdate(whXh){
    	var url = jcontextPath+"/zygl/qythshdz!initMx.action?domain.whXh="+whXh;
    	window.showModalDialog(url,window,"dialogHeight:420px;dialogWidth:600px;center:yes;resizable:no;status:no;scroll:yes;help:no;")
    	onRefresh();
    }
	...
	function yesCallBack(){
		 ...
		 var url = jcontextPath+"/zygl/qythshdz!delete";   
	...
	function onRefresh(){
		var ssJgbm = $("#mainForm_domain_ssJgbm").val();
		//请求表格数据
		var url = jcontextPath+"/zygl/qythshdz!query.action";   
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '车辆型号维护序号(SEQ_ZY_DJXH)','所属机构','地址','联系电话','负责人','行政区划',
		              '创建人','创建日期','修改人',
				     '修改日期'],			 //name 列显示的名称
		     //name 对应结果集里面的属性名称  index 传到服务器端用来排序用的列名称 width 列宽度；align 对齐方式；sortable   是否可以排序
		    colModel :[ 
			  {name:'hstoperationcol', index:'', sortable:false, width:'60', align:'center'},
		      {name:'whXh', index:'whXh', width:'100', align:'center',hidden:true}, 
		      {name:'ssJgmc', index:'ssJgmc', width:'150', align:'left'}, 
		      {name:'dz', index:'dz', width:'200', align:'left'}, 
		      {name:'dh', index:'dh', width:'100', align:'center'}, 
		      {name:'fzr', index:'fzr', width:'80', align:'center'}, 
			  {name:'fhrXzqhMc', index:'fhrXzqhMc', width:'100', align:'center'}, 
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/zygl/qythshdz!download");
				   $("#mainForm").submit();
		       } 
		 });
...
<body>
<s:form action="qythshdz!query" namespace="/zygl" method="post" id="mainForm" name="mainForm">
		...
		<div id="divQuery">
	    		<fieldset>
		      		...
		      		<table width="30%" border="0" cellspacing="0" cellpadding="0">
		        		<tr>
		          			<td width="5%" align="right">所属单位：</td>
		          			<td width="10%">
