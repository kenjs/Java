...
<script type="text/javascript">
	$(document).ready(function(){
			...
			var khDjxh = trim($("#mainForm_domain_fhrDjxh").val());  
				...
				var url = jcontextPath+"/zygl/qykhzhdz!initMx.action?domain.ssJgbm="+ssJgbm+"&domain.khDjxh="+khDjxh;
    ...
    function onUpdate(zhdzDjxh){
    	var url = jcontextPath+"/zygl/qykhzhdz!initMx.action?domain.zhdzDjxh="+zhdzDjxh;
		    	window.showModalDialog(url,window,"dialogHeight:380px;dialogWidth:640px;center:yes;resizable:no;status:no;scroll:yes;help:no;")
		    	onRefresh();
    }
    
    var keyValue = "";
	function onDelete(zhdzDjxh){
		keyValue = zhdzDjxh;	
		showConfirm("确定要删除么？","yesCallBack");
	}	
	function yesCallBack(){
		 var jsonObj = {"domain.zhdzDjxh":keyValue};
		 var url = jcontextPath+"/zygl/qykhzhdz!delete";  
	...
	function onRefresh(){
	   ...
	   var khDjxh = trim($("#mainForm_domain_fhrDjxh").val()); 
		...
		//请求表格数据
		var url = jcontextPath+"/zygl/qykhzhdz!query.action";   
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作','客户名称','单位名称','部门名称','行政区划','详细地址','邮编','联系人',
		              '移动电话','固定电话','其他联系电话','备注',
		    ...
		    colModel :[ 
		      ...
		      {name:'zhdzDjxh', index:'zhdzDjxh', width:'0', align:'center',hidden:true}
		    ],
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/zygl/qykhzhdz!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var val = jQuery("#dataList").jqGrid('getCell', cl,"zhdzDjxh"); 	  //获取当前单元格里面的登记序号 
...
<body>
<s:form action="qykhzhdz!query" namespace="/zygl" method="post" id="mainForm" name="mainForm">
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0">
		<tr>
	          ...
	          <td width="21%">
			  		...
			  		<div class="inputsc">
		              <div id="inputSel_fhr" class="inputselcont inputselFixedSize ac_results" style="width:300px;">
		              </div>
		            </div>
		          </td>
	        </tr>
