...
<script type="text/javascript">
	...
	function onRefresh(){
		...
		var url = jcontextPath+"/zygl/qykhhwxx!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.ssJgbm":ssJgbm,"domain.djJgbm":djJgbm,"domain.khDjxh":khDjxh,"domain.fhrMc":encodeURI(str)}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '物流登记序号','客户名称','货物名称','货物简称',
				     '拼音全称','拼音简称','长度计量单位','长度','宽度',
				     '高度','数量计量单位','重量计量单位','体积计量单位','包装计量单位','折结算计量单位换算比例','包装折成本计量单位换算比例','结算计量单位',
				     '备注','登记部门','登记人','登记日期',
				     '启用标志(Y/N)','有效标志(Y/N)','创建人','创建日期','修改人',
				     '修改日期'],			 //name 列显示的名称
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var val = jQuery("#dataList").jqGrid('getCell', cl,"hwDjxh"); 	  //获取当前单元格里面的登记序号 
