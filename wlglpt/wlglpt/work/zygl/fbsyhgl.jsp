...
<script type="text/javascript">
   ...
   function stop(yhDjxh){
   		keyValue=yhDjxh
   		showConfirm("您确定要停用该用户么？","stopCallBack");
   }  
   ...
   function start(yhDjxh){
    	keyValue=yhDjxh
   		showConfirm("您确定要启用该用户么？","startCallBack");
   }   
   ...
   function startSuccess(){
	...
	function onRefresh(){
		...
		var url = jcontextPath+"/zygl/fbsyhgl!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.ssJgbm":ssJgbm,"domain.fbsDjxh":encodeURI(fbsDjxh)}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '状态', '名称', '账号','登录验证方式','创建人','创建日期',
		              '修改人','修改日期','主键'],			 //name 列显示的名称
		    ...
		    sortname: 'YH_DJXH',				//默认排序 可以是列名称或者是一个数字，这个参数会被提交到后台
	...
	function myGridComplete() {
         ...
         for (var i = 0; i < graduateIds.length; i++) {
             ...
             var val = jQuery("#dataList").jqGrid('getCell', cl,"yhDjxh"); 	  //获取当前单元格里面的登记序号 
