...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#addBtn").click(function(){
				...
				var url = jcontextPath+"/cwgl/fylbwh!initMx.action";
    ...
    function onUpdate(cwDjxh){
    	...
    	var url = jcontextPath+"/cwgl/fylbwh!initMx.action?domain.cwDjxh="+cwDjxh;
		window.showModalDialog(url,window,"dialogHeight:400px;dialogWidth:650px;center:yes;resizable:no;status:no;scroll:yes;help:no;")
		onRefresh();
    }
	...
	function yesCallBack(){
		 ...
		 var url = jcontextPath+"/fylbwh!delete";   
	...
	//刷新当前页面
	function onRefresh(){
		 var lb=$("#mainForm_domain_fylbMc").val();
		//请求表格数据
		var url = jcontextPath+"/fylbwh!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.fylbMc":encodeURI(lb)}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['操作', '财务登记序号(SEQ_CW_DJXH)','所属机构','费用类别名称','创建人',
		    ...
		    colModel :[ 
		      ...
		      {name:'fylbMc', index:'fylbMc', width:'100', align:'center'}, 
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/fylbwh!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	<fieldset>
		<legend>查询条件</legend>
		   <table width="95%" border="0" cellspacing="0" cellpadding="0">
	        <tr>
			<td width="12%" align="right">费用类别名称：</td>
			<td width="30%">
				
				<s:textfield name="domain.fylbMc" cssClass="pop_input noborder" />
