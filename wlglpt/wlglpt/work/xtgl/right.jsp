...
<style type="text/css">
<!--
html,body {background-color:#eff8fd;}
.tab {font-size:12px; color:#fff; background:url(<sys:context/>/resource/wlglpt/images/tab2.jpg) no-repeat; cursor:pointer;}
.tab_sel {font-weight:bold; font-size:12px; color:#000; cursor:pointer; background:url(<sys:context/>/resource/wlglpt/images/tab1.jpg) no-repeat;}
...
function yesCallBackOne(){
	parent.document.location.href=jcontextPath+"/login!logout.action";  
}
 ...
 */
 function flagDir(){
	...
	toLeftObject.src = "<sys:context/>/resource/pageframe/images/arrow1.gif";
	toRightObject.src = "<sys:context/>/resource/pageframe/images/arrow2.gif";
	//
...
function scroll_doMouseOver(obj){
	...
	if(obj.src.lastIndexOf("arrow"+flag+"_2.png") != -1){
		return;
	}
	obj.src = "<sys:context/>/resource/pageframe/images/arrow"+flag+"_1.gif";
}
...
function scroll_doMouseOut(obj){
	...
	obj.src = "<sys:context/>/resource/pageframe/images/arrow"+flag+".gif";
}
...
function addTab(sTitle,sIcon,sUrl){
	...
	oCel.noWrap = true;
	oCel.title = sTitle;
	oCel.style.width="115px";
	oCel.style.textAlign="left";
	oCel.style.paddingTop="0";
	oCel.className = "tab";
    //兼容IE及Firefox
	...
	oCel.innerHTML = "<label style='width:100px; text-align:center; display:inline-block;' oncontextmenu=\"return false;\" onselectstart=\"return false;\">"+titleSegment+"&nbsp;</label><img src='<sys:context/>/resource/pageframe/images/close_out.gif' onmouseover='src=\"<sys:context/>/resource/pageframe/images/close_over.gif\"' onmouseout='src=\"<sys:context/>/resource/pageframe/images/close_out.gif\"' onclick=\"closeWork(tab_doClick())\" />";
...
function navigate(sUrl,sTitle,bl){
	...
	if(getTabCount() < MAX_TAB_COUNT){
		addTab(sTitle,"<sys:context/>/resource/pageframe/images/state1.gif",sUrl);
	} else{
...
function resizeWork(obj){
    ...
    if(wholeObject.rows == sizeWholeOrg){
		...
		obj.src = "<sys:context/>/resource/pageframe/images/min.gif";
		//obj.innerHTML = "2";
		obj.title = "还原";
	}else{
		wholeObject.rows = sizeWholeOrg;
		middleObject.cols = sizeMiddleOld;//
		obj.src = "<sys:context/>/resource/pageframe/images/max.gif";
		obj.title = "工作区全屏";
	}
...
function removeTabAt(idx){
	...
	setTimeout("removeTabParams("+idx+")",100);
	//removeTabParams(idx);
	setTimeout("removePanelsAt("+idx+")",100);
	//removePanelsAt(idx);
	if(idx < SELECTED_INDEX){
		SELECTED_INDEX --;
	}else if(idx == SELECTED_INDEX){
		SELECTED_INDEX = -1;
		var count=tabListObject.cells.length-1;
		setTimeout("setSelectedIndex("+count+")",100);
...
<body style="overflow:hidden; margin:0; padding:0px; padding-top:27px; height:100%;">
<!-- 页面右侧最上面显示tab页区域 -->

<table border="0" cellspacing="0" cellpadding="1" class="right_top">
  <tr>
    <td><div id="tabContainer" style="position:relative; top:2px; width:100%; height:25px; font-size:12px; overflow:hidden; overflow-x:hidden;" oncontextmenu="return false;" onselectstart="return false;">
        <table style="height:25px; table-layout:fixed;" border="0" cellSpacing="0" cellPadding="0">
          <tr id="tabList"> </tr>
        </table>
      </div></td>
    <td width="20"><img src="<sys:context/>/resource/pageframe/images/arrow1.gif" id="toLeft" style="cursor:pointer;" onMouseOver="scroll_doMouseOver(this)" onMouseOut="scroll_doMouseOut(this)" alt="向左滚动" onClick="scrollTab('PageLeft')" /></td>
    <td width="20"><img src="<sys:context/>/resource/pageframe/images/arrow2.gif" id="toRight" style="cursor:pointer;" onMouseOver="scroll_doMouseOver(this)"
onMouseOut="scroll_doMouseOut(this)" alt="向右滚动" onClick="scrollTab('PageRight')" /></td>
	<td width="20"><img src="<sys:context/>/resource/pageframe/images/max.gif" alt="工作区全屏" style="cursor:pointer" onClick="resizeWork(this)" id="maxImg" /></td>
	<!--
    <td width="20"><img src="<sys:context/>/resource/pageframe/images/folder_up.gif" alt="开/闭权限树" style="cursor:pointer;" onClick="switchLeft();" id="folderImg" /></td>
    -->
  </tr>
</table>
<!-- 页面右侧下面显示主内容区域 -->
