...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#addBtn").click(function(){
				...
				if(sjJgbm==""||sjJgbm==undefined)
				{
					showError("请选择公司/分公司!");
					return false;
				}
				var url = jcontextPath+"/qyzzjg!initMx.action?domain.sjJgbm="+sjJgbm;
		    	window.showModalDialog(url,window,"dialogHeight:420px;dialogWidth:620px;center:yes;resizable:no;status:no;scroll:yes;help:no;")
		    	onRefresh();
			});
			//初始化表格
			initDataGrid();
 ...
 /**************************************分页开始****************************************/
	...
	function onRefresh(){
		var sjJgbm = trim($("#mainForm_domain_sjJgbm").val()); 
  		//请求表格数据
		var url = jcontextPath+"/xtgl/qyzzjg!query.action";   
		 $("#dataList").jqGrid("setGridParam",{
		 	url:url,
		 	datatype:'json',
		 	postData:{"domain.sjJgbm":sjJgbm,"domain.yxbz":"Y"}								//请求的参数，json格式
		 }
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    colNames:['操作','ID','级别编码','状态','类别','名称',
		    		  '简称','备用金','上级单位','行政区划','地址','负责人','电话','创建人',
		    ...
		    colModel :[ 
			 {name:'hstoperationcol', index:'', sortable:false, width:'100', align:'center'},
		      ...
		      {name:'jc', index:'jc', width:'130', align:'center'},
		      {name:'byj', index:'byj', width:'60', align:'center'}, 
		      {name:'sjMc', index:'sjMc', width:'130', align:'center'}, 
		      {name:'fhrXzqhMc', index:'fhrXzqhMc', width:'130', align:'center'}, 
		      {name:'dz', index:'dz', width:'200', align:'left'}, 
		  ...
		  $("#dataList").jqGrid('navButtonAdd','#pager',{
		       ...
		       onClickButton : function () { 
		       	   $("#mainForm").attr("action",jcontextPath+"/xtgl/qyzzjg!download");
				   $("#mainForm").submit();
		       } 
		 });
	...
	function myGridComplete() {
            ...
            for (var i = 0; i < graduateIds.length; i++) {
                ...
                var jbdm = jQuery("#dataList").jqGrid('getCell', cl,"jbdm"); 
   ...
   function stopCallBack(){
        var url=jcontextPath+"/qyzzjg!saveDisable";
  ...
  function stopSuccess(){
   ...
   function startCallBack(){
   		var url=jcontextPath+"/qyzzjg!saveEnable";
  ...
  function onUpdate(id){
 			var url = jcontextPath+"/qyzzjg!initMx.action?domain.jgbm="+id;
		    window.showModalDialog(url,window,"dialogHeight:420px;dialogWidth:620px;center:yes;resizable:no;status:no;scroll:yes;help:no;")
		    onRefresh();
  }
  ...
  function yesCallBack(){
		var url=jcontextPath+"/qyzzjg!delete";
...
<body>
<s:form action="qyzzjg!query" namespace="/xtgl" method="post" id="mainForm" name="mainForm">
