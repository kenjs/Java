$(function(){
	...
	function checkYfxx() {
		var pcfsDm = $("#mainForm_domain_pcfsDm").val();
		if(pcfsDm=="3"){
			if($("#mainForm_domain_yfYfyf").val()/1 + $("#mainForm_domain_yfHdyf").val()/1 + $("#mainForm_domain_yfHdf").val()/1 != $("#mainForm_domain_yfHj").val()/1){
				alert("必须符合公式：运费=预付+到付+回付！");
			 	return false;
			 }
		}else{
	...
	function saveWfhxx4PcSuc(data) {
		...
		var qrPsf = data.domain.qrPsf;
		if ($("#mainForm_domain_pchwLsxh").val() == "") {
			$("#mainForm_domain_pchwLsxh").val(pchwLsxh)
		}
		var yfHj = $("#mainForm_domain_yfHj").val();
		var yfYfyf = $("#mainForm_domain_yfYfyf").val();
		
		$("#mainForm_domain_yfHj").val(yfHj/1+qrPsf/1);
		$("#mainForm_domain_yfYfyf").val(yfYfyf/1+qrPsf/1);
		// 清空支出信息
	...
	function checkZrDzdata(){
		...
		// 运输派车时必须选择一个下游，如果转到分包商时必须选择一个分包商,转包时必须选择下游(专线时才判断)
		if (zrbmDjxh == "" && xtcs20000=="2") {
