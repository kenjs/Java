...
/**
 * 
* @Descriptoin 总公司下所有分公司下拉列表
* @Note
* @author admin
* @version
 */

public class FgsListTag extends TagSupport {
	...
	private String contaisDq; //是否包含当前操作员所在公司(Y或N)
	...
	public int doEndTag() {
		try {
			...
			map.put("zgsJgbm", userDomain.zgsbm);
			map.put("ssGsbm", userDomain.gsbm);
			map.put("mcContainDmBz", mcContainDmBz);
			map.put("contaisDq", contaisDq);
			List<DmbGgDomain> dataList = businessSqlMapClientTemplate.queryForList("getFgsDdlbList", map);
	...
	public String getContaisDq() {
		return contaisDq;
	}

	public void setContaisDq(String contaisDq) {
		this.contaisDq = contaisDq;
	}
}