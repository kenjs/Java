...
public class GsBmListTag extends TagSupport {
		...
		try {
			...
			for(Iterator iter=dataList.iterator();iter.hasNext();){
				...
				if(domain.getDm().equals(defaultValue)){
					pageContext.getOut().println("<option value=\""+domain.getDm()+"\" selected=\"selected\">"+domain.getMc().replace(" ", "&nbsp;")+"</option>");
				}else{
					pageContext.getOut().println("<option value=\""+domain.getDm()+"\">"+domain.getMc().replace(" ", "&nbsp;")+"</option>");
				}
			}
