 ...
 */
@Repository
public class JcShQkTjDaoImp extends ExtendDaoImp implements JcShQkTjDao {
	@Autowired
	...
	public List<BaseBusinessDomain> queryList(BaseBusinessDomain baseDomain) throws Exception {
		JcShQkTjDomain domain=(JcShQkTjDomain)baseDomain;
		...
		map.put("date", domain.getTjDate());
		map.put("fhrDjxh", domain.getFhrDjxh());
		String fhrMc=SysEncodeUtil.GBK2ISO(domain.getFhrMc());
		map.put("fhrMc", fhrMc);
		map.put("dataList", dataList);
		
		businessSqlMapClientTemplate.queryForObjectByCurr("queryShQkTjList", "dataList", map);
