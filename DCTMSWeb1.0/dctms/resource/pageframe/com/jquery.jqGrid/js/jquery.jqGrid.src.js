...
//jqGrid请求响应有错误时调用方法
function hstError(data){
//	alert(data.responseText);
	showError(data.responseText);  //在helper.js中定义的方法
}
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			//modified by anq 去掉页面输出控制，使得返回的datalist里的所有行均能显示
			//var rn = parseInt(ts.p.rowNum,10),br=ts.p.scroll?$.jgrid.randId():1, altr;
			var rn = 9999999,br=ts.p.scroll?$.jgrid.randId():1, altr;
			if (adjust) { rn *= adjust+1; }
			...
			if(!ts.p.treeGrid && !ts.p.scroll) {ts.grid.bDiv.scrollTop = 0;}
			//modified by anq 自定义一个reccount传入，即当前页的实际数据条数
			//ts.p.reccount=ir;
			var localReccount = $.jgrid.getAccessor(data,dReader.reccount);
			if (localReccount) {
				ts.p.reccount=localReccount;
			}else {
				ts.p.reccount=ir;
			}
			ts.p.treeANode = -1;
        ...
        setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>每页</td>";
				str +="<td><select class='ui-pg-selbox' role='listbox'>";
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-pg-bf'>首页</span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='ui-pg-bf'>上一页</span></td>";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='ui-pg-bf'>下一页</span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-pg-bf'>尾页</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";			//第几页共几页
