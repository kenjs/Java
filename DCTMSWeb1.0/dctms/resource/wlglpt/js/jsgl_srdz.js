$(function(){
    ...
    function onUpdate(jsDjxh,dzDjxh,dzfsDm){
    	var url = jcontextPath+"/hygl/jsglsrdz!initMx.action?domain.jsSrdzDomain.jsDjxh="+jsDjxh+"&domain.jsSrdzDomain.dzDjxh="+dzDjxh+"&domain.jsSrdzDomain.dzfsDm="+dzfsDm;
	...
	function initDataGrid(){ 
		  $("#dataList").jqGrid({
		    ...
		    shrinkToFit:false, 
		    colNames:['序号','<input title="选中/取消选中" type="checkbox" onclick="checkAllJGridChk(event,this,\'xhs\');" />','操作',
		    		  '文书审批序号','文书审批状态dm','审核',
		    ...
		    colModel :[ 
			  ...
			  {name:'checkboxoperationcol', index:'checkboxoperationcol', sortable:false, width:'30', align:'center', 
				cellattr: function(rowId, tv, rawObject, cm, rdata) {
	...
	function myGridComplete() {
       ...
       //按订单
       if("1"==dzfsDm){
       		var a = ['pageXh','checkboxoperationcol','hstoperationcol','wsSpztDm','wsspxh','wsSpztMc','jsDjxh','dzDjxh','dzztDm','dzztMc','khMc',
   ...
   //jqGrid  初始化表格(按订单货物)
	function initDataGridByDdHw(){ 
		  $("#dataList").jqGrid({
		    ...
		    colNames:['序号','<input title="选中/取消选中" type="checkbox" onclick="checkAllJGridChk(event,this,\'xhs\');" />','操作',
		    		  '文书审批序号','文书审批状态dm','审核',
		    		  '结算登记序号','对账登记序号','状态dm','状态','客户名称','未结','对账金额','结果','差异金额','ddDjxh','订单编号','下单日期', 
		    		  '货物名称', '结算数量', '回单编号','始发地', '目的地', '数量', '重量', '体积','包装',
		    		  '对帐人','对帐日期','对账部门','业务单位'
		              ],			 //name 列显示的名称
		     //name 对应结果集里面的属性名称  index 传到服务器端用来排序用的列名称 width 列宽度；align 对齐方式；sortable   是否可以排序
