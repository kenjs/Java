...
import com.cy.swp.dao.CommonDao;
...
import com.cy.swp.common.util.HttpPostUtil;
import com.cy.swp.dao.MarketingNoteRecordDao;
...
public class MarketingNoteRecordServiceImpl implements MarketingNoteRecordService {
	...
	public String setNoteSendRecordInfo(String type, String remark, String telephone, String content, String useFor) {
		...
		try {
			//发送短信
			String returnStr = getNoteNb(telephone, content);
	...
	public String setPushInfo(String driverId, String title, String message, String id, String type, String spKey) {
		...
		try {
			...
			String returnStr = getPush(driverId, title, message, id, type, pushChannel);
			ObjectMapper mapper = new ObjectMapper();
			JsonNode node = mapper.readTree(returnStr);
            //推送接口发送返回状态 0失败，1成功
			...
			pushSendRecord.setEventFrom(Constants.EVEN_FROM_MARKETING_KEY);
...
//    
     ...
     * @return
     */
    public String getNoteNb(String mobilephone, String noteCode) {
		...
		params.put("eventFrom" , "1");
        return HttpPostUtil.doPostRequest(posturl.toString(), params);

    }

	public String getPush(String driverId, String title, String message, String id, String type, String pushChannel) {
        ...
        params.put("eventFrom", Constants.EVEN_FROM_MARKETING_KEY + "");//推送请求来源 1 营销平台 2 快到网网站 3 app服务端
    ...
    public Integer sendNoteSDKService(String mobilephone,String content) {
		...
		try {
			//发送短信
			String returnStr = getNoteNb(mobilephone, content);
			ObjectMapper mapper = new ObjectMapper();
	        JsonNode node = mapper.readTree(returnStr);
     ...
     */
    @Override
    public boolean sendTextMessageRecord(String type, String remark, String telephone, String content, String useFor) {
        int sutra  = 1;//失败
        try {
			sutra = 0;
            //发送短信
            ...
            noteSendRecord.setTelephone(telephone);
            noteSendRecord.setContent(content);
            noteSendRecord.setEventFrom(Constants.EVEN_FROM_MARKETING_KEY + "");
			noteSendRecord.setNoteSendedId(String.valueOf(pushId));
            noteSendRecord.setReturnStatus(String.valueOf(sutra));
            noteSendRecord.setRemark(remark);//被发送短信的人的号码保存到备注中
	 ...
	 */
	@Override
	public boolean setTextMessagePush(String driverId, String title, String message, String id, String type, String spKey) {
		int sutra = 1;//失败
		try {
			//查询系统参数   推送渠道
			String pushChannel = querySysCodeServ(Constants.CODE_PUSH_CHANNEL, spKey, "0");
    ...
    @Value("#{propertiesReader['dctms.download.sendNoteNbSDK']}")
    public void sendNoteNbSDK(String sendNoteNbSDK) {
        this.sendNoteNbSDK = sendNoteNbSDK;
    }

	@Value("#{propertiesReader['dctms.download.sendPushSDK']}")
