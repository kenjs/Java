...
var pa_pageSize = 12;//每页显示个数
var pa_currentPage = 1;

function sendAllProvinceAjax() {
	$.ajax({
		type: "get",
		url: '../js/sys/CityPicker/queryAllProvinces.js',
		async: false,
		dataType: "json",
		success: function(data) {
...
function sendAllCitiesAjax() {
	$.ajax({
		type: "get",
		url: '../js/sys/CityPicker/queryCities.js',
		async: false,//false为用户必须请求完成之后才能执行另外的操作;true,则不会等待ajax请求返回的结果，会直接执行ajax后面的语句。
		dataType: "json",
		success: function(data) {
			allCities = data.cities;//将所返回的cities参数中的数据赋给allCities变量
			$("body").data("CitysAll", data);//将所返回的数据赋值加给body元素的CitysAll变量.
			viewAllHotCities();
		},
		error: function(XMLHttpRequest, textStatus, errorThrown)
		 {
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "get",
		url: "../js/sys/CityPicker/queryAllAreas.js",
		async: false,
		dataType: "json",
		success: function(data) {
			allAreas = data.areas;//将所返回的areas参数中的数据赋给allAreas变量
			$("body").data("allCountys", data.areas);//将所返回的数据赋值加给body元素的allCountys变量.
		},
		error: function(XMLHttpRequest, textStatus, errorThrown)
		 {
			alert("网络繁忙，请稍后再试！");
...
function viewAllHotCities() {
	...
	function(i, city) {
		...
		}//遍历所有的城市，如果城市中的hotCity属性为ture责将其显示出来.
	});
}//显示热门城市
