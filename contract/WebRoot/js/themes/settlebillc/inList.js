...
function f_hgrid_ini(type) {//初始化表格
	...
	var txt='<tr>' +
		...
		'<th scope="col"   width="70px" class="tdleft">应收合计</th>' +
		'<th scope="col"   width="140px" class="tdleft">最后编辑时间</th>' +
		'<th scope="col"   width="90px" class="tdleft">操作</th>' +
		'</tr>';
...
function f_hgrid_json(param) {//刷新hGrid数据
	...
	if(organization=="发货方"){
		...
		param=f_hgrid_getparam(param)+"&inorout="+inorout; //得到全部参数
	 	$.ajax({
	 	   	...
	 	   	success:function(data){//回传函数
	 			...
	 			for(var i =0;i<data.length;i++){ //展现返回的表格数据
	 				if(data[i].status=="未审核"){
	 					operate='<a href="javascript:void(0)" onclick="update(this,'+'&quot;'+data[i].settlebillid+'&quot;,&quot;'+data[i].organization+'&quot;,&quot;'+data[i].inoutpartyid+'&quot;,&quot;'+i+'&quot;)">修改</a>'+
	 					'&nbsp;<a href="javascript:void(0)" onclick="deleteAction(this'+',&quot;'+data[i].settlebillid+'&quot;,&quot;'+i+'&quot;)">删除</a>';
	 				}else{
	 					operate='';
	 				}
	 				txt='<td class="tdleft">&nbsp;<a href="javascript:void(0)" onclick="detail(&quot;'+data[i].settlebillid+'&quot;,&quot;'+data[i].settlebillnumber+'&quot;,&quot;'+data[i].startdate+'-'+data[i].enddate+'&quot;,&quot;'+data[i].organization+'&quot;,&quot;'+i+'&quot;,&quot;'+inorout+'&quot;)" style="margin-left:0px;">'+data[i].settlebillnumber+'</a></td>'+
	 					...
	 					'<td class="tdleft"><span style="margin-left:0px;" id="tr'+i+'_td7">'+data[i].inputdate+'</span></td>'+
	 				 	'<td class="tdleft">&nbsp;'+
	 				 	'<a href="javascript:void(0)" onclick="detail(&quot;'+data[i].settlebillid+'&quot;,&quot;'+data[i].settlebillnumber+'&quot;,&quot;'+data[i].startdate+'-'+data[i].enddate+'&quot;,&quot;'+data[i].organization+'&quot;,&quot;'+i+'&quot;,&quot;'+inorout+'&quot;)">详情&nbsp;</a>'+
	 				 	operate+'</td>';
...
function getConditions(){
	...
	var params="clientnumber="+clientnumber+"&settlebillnumber="+settlebillnumber+"&startdate="+startdate+"&enddate="+enddate
				+"&inoutpartyrealname="+inoutpartyrealname+"&random="+Math.random();
	return params;
}
...
function update(obj,settlebillid,organization,inoutpartyid,i){
	var params="&settlebillid="+settlebillid+"&organization="+encodeURI(organization)+"&inoutpartyid="+inoutpartyid+"&random="+Math.random();
	window.location="../settlebillcs/inUpdate?order=7"+params;
}
...
function detail(settlebillid,settlebillnumber,start_end_date,organization,trnum,inorout){
	var status=$("#tr"+trnum+"_td2").html();
	var data=encodeURI(settlebillid+","+settlebillnumber+","+status+","+start_end_date+","+organization+","+inorout);
