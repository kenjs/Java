	function add(){
		...
		var consignee=$("#consignee").attr("myid");
		if(consignor!=""&&$("#consignor").val()!=""&&consignee!=""&&$("#consignee").val()!=""){
			ymPrompt.win({message:'../settlebillcs/outAddWayBill?frompartyid='+consignor+"&type=1&inoutpartyidsec="+consignee,width:820,height:500,fixPosition:true,dragOut:false,title:'添加托运单',iframe:true});
		}else{
			ymPrompt.alert({title:"提示",message:"请正确填写分包商和发货方"});
			return false;
		}
		}
	var settleBillId;
			...
			if(status=='ok'){
				...
				$.ajax({
					url: "../settlebillcs/deleteWayBillOfSettleBill",   
				 	type:'post',	
				 	dataType:'json', 
				 	data:{"settlebillid":settleBillId,"waybillid":waybillId,"inorout":"0","action":operator}, //参数     	               
				   	success:function(data){//回传函数
