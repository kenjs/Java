...
$(function(){
	$("#consignor").html(resquest["organization"]);//分包商
	$("#consignee").html(resquest["conorganization"]);//发货方
	addedList(settleBillId);
});
function add(){
		ymPrompt.win({message:'../settlebillcs/outAddWayBill?frompartyid='+resquest['inoutpartyid']+"&type=0"+"&settlebillid="+resquest['settlebillid'],width:820,height:500,fixPosition:true,dragOut:false,title:'添加托运单',iframe:true});
	}
			...
			if(status=='ok'){
				...
				$.ajax({
					url: "../settlebillcs/deleteWayBillOfSettleBill",   
				 	type:'post',	
				 	dataType:'json', 
				 	data:{"settlebillid":settleBillId,"waybillid":waybillId,"action":operator,"inorout":"0"}, //参数     	               
				   	success:function(data){//回传函数
						...
						var enddate2=$(consigndate1[length-1]).html();
				 		if(length1==0){
				 			window.location=("../settlebillcs/outList?order=8");
				 		}else{
	...
	function selectList(obj){
		...
		$.ajax({
			url: "../settlebillcs/wayBillOfSettleBillListWithCondition",   
		 	type:'post',	
		 	dataType:'json', 
		 	data:{"settlebillid":settleBillId,"inorout":"0",
	...
	function updateWayBillAmount(settleBillId,waybillId){
		...
		window.location.href="../waybillamountcs/waybillamountUpdate?order=8&waybillid="+waybillId+"&settlebillid="+settleBillId+"&returnUrl"+url_Data;
	}