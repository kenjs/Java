...
function btnSave(){
	...
	$.ajax({
		url:'../dispatchtrackcs/carbeginconfirm_save',
		type:'post',
		dataType:'json',
		data:{"waybillstowageid":waybillstowageid,"carbegindate":carbegindate,"systemdispatchnumber":systemdispatchnumber},
		success:function(data){
			if(data.msg=='ok'){
				window.parent.ymPrompt.doHandler('begin_ok',true);
			}else{
				window.parent.ymPrompt.doHandler('begin_fail',true);
			}
		},
		error:function(){
			window.parent.ymPrompt.doHandler('begin_fail',true);
