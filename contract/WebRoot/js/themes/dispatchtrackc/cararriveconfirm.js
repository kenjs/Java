...
function btnSave(){
	var waybillstowageid=document.URL.split("?")[1].split("&")[0].split("=")[1],
		...
		systemdispatchnumber=document.URL.split("?")[1].split("&")[1].split("=")[1];
	$.ajax({
		url:'../dispatchtrackcs/cararriveconfirm_save',
		type:'post',
		dataType:'json',
		data:{"waybillstowageid":waybillstowageid,"carenddate":carbegindate,"systemdispatchnumber":systemdispatchnumber},
		success:function(data){
			if(data.msg=='ok'){
				window.parent.ymPrompt.doHandler('arrive_ok',true);
				//ymPrompt.alert({message:"到车确认成功！",handler:closeBack});
			}else{
				window.parent.ymPrompt.doHandler('arrive_fail',true);
			}
		},
		error:function(){
			window.parent.ymPrompt.doHandler('arrive_fail',true);
