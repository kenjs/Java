		...
		/*	if(data.list.length>0){
			...
			if(data.list.length>0){
				...
				/*txt=txt+'<tr style="height:25px;"><td class="tdleft"><a href="../attachment/'+data.list[i].filename+'" title="下载" style="color:#1560ea;">'+data.list[i].filename+'</a></td></tr>';*/
					txt=txt+'<tr style="height:25px;"><td class="tdleft"><a href="'+data.list[i].url+'" title="下载" style="">'+data.list[i].filename+'&nbsp;下载'+'</a></td></tr>';
			}
				$("#myfiles").append(txt);
			}
		}
...
function btnSubmit(obj){
	...
	var params1="pactnumber="+trim($("#pactnumber").val())+"&type="+trim($("#type").val())+"&frompartyrealname="+trim($("#frompartyrealname").val())+
				...
				"&memo="+trim($("#memo").val())+"&signdate="+trim($("#signdate").val())+"&pactid="+$('#pactid').val()+"&frompartyid="+frompartyid+"&topartyid="+topartyid+"&random="+Math.random();
	$.ajax( {
