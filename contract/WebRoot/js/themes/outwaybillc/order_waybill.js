...
function tabendAdd(){
	 ...
	 var hgrid_tr=$("<tr>").attr({"style":"height:40px;"}); //创建tr
				...
				showOverlay();
				 $.ajax({
						...
						data:{jsonText:jsonText,
						 ...
						 sourcesystem:trim($("#sourcesystem").val()),
						 memo:trim($("#memo").val()),
...
function saveOrderWaybill(){
	$.ajax({
			url: "../outwaybillcs/save_order_waybill",   
			type:'post',	
			dataType:'json', 
			data:{
			waybillid:trim($("#primarykeyid").val()),
			contractWaybillId:trim($("#waybillid").val()),
			waybillnumber:trim($("#waybillnumber").val())
			}, //参数     	               
			success:function(data){//回传函数
				 //alert(data.goodstype)
				if(data!=null&&data.msg=='ok'){
					//遮罩影藏
			 			hideOverlay();
			 			ymPrompt.alert({message:'订单确认并保存成功！',handler:f_fanhui});
	...
	$.ajax({
		url: "../outwaybillcs/oreder_detail_json",   
	 	...
	 	data:{"waybillid":param}, //参数     	               
	   	success:function(data){//回传函数
	   		$("#primarykeyid").val(data.waybillid);
	 		 $("#waybillid").val(data.contractWaybillId);
			 $("#frompartyid").attr("myid",data.frompartyid);
			 $("#frompartyid").val(data.consignor);
			 $("#partyid").val(data.partyid);
			 $("#waybillnumber").val(data.waybillnumber);
			 $("#status").val(data.status);
			 ...
			 $("#memo").val(data.memo);
			 $("#sourcesystem").val(data.sourcesystem);
//			 $("#inputman").val(data.inputman);
...
//			 $("#suremanDiv").html(data.sureman);
			 var datae = eval(data.outGoodsList);
			 var numZ = 0;
			 var weightZ = 0;
			 var volumeZ = 0;
			 for(var i=0;i<datae.length;i++) {
