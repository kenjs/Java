...
(function() {
	...
	(function() {
		...
		var winVisible = function(visible) {
			...
			if (curCfg.iframe) {
				var attrs = "";
				if (typeof curCfg.iframe == "object") {
					for (var k in curCfg.iframe)
						attrs += ' ' + k + '="' + curCfg.iframe[k] + '"';
				}
				ym_body.innerHTML = '<iframe' + attrs + ' width="100%" height="100%" border="0" frameborder="0" src="' + curCfg.message + '"></iframe>';
			} else {
				ym_body.innerHTML = '<div class="ym-content">' + curCfg.message + '</div>';
			}
