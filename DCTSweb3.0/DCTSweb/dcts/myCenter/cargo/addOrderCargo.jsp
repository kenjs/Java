                ...
                success: function(data) {
					  if(data.result=='0'){
					  	getOpenes(data.content);
					   	//art.dialog.open('<sys:context/>/dcts/cargo/addOrderCargoInfoSuccess.jsp?orderCargoId='+data.content,{width:437,height:228,lock:true,drag:true});//drag 是否允许用户拖动
   ...
   <input type="hidden" id="id" name="orderCargoInfoDomain.id"  value="${orderCargoInfoDomain.id }"/>
    ...
    <div class="laing shng">
        <ul>
          ...
          <li>
            <label>详细地址</label>
            <input name="orderCargoInfoDomain.startTown" value="${orderCargoInfoDomain.startTown }" type="text" class="int" style="width: 330px;"/>
          </li>
          <li>
          ...
          <li>
            <label>详细地址</label>
            <input name="orderCargoInfoDomain.endTown" value="${orderCargoInfoDomain.endTown }" type="text" class="int" style="width: 330px;"/>
          </li>
         <li class="fkl">
            <label>货物信息&nbsp;</label>
          ...
          <li>
            ...
            <div class="mpt" style="display:none" id="contactMobilephoneMpt">
             ...
             <div class="remark_info" >注：手机号码或固定电话必须填写一个。</div>
          </li>
          <li>
            <label>固定电话</label>
          ...
          <li class="ternow">
            <label></label>
            <input name="" type="button" onclick="getSubmit()" value="提交" class="sub" />&nbsp;&nbsp;
            <input class="sub"  type="button" onclick="locationHref('<sys:context/>/dcts/myCenter/cargo/importOrderCargoFromFile.jsp?menuAId=<s:property value='@com.cy.dcts.common.constants.Constants@PUBLISH_CARGO'/>');" value="导入货源" />
            </li>
        </ul>
...
<!-- 未登录时弹出快捷登录框 
<script type="text/javascript" src="<sys:context/>/resource/js/index/login.js"></script>
<script type="text/javascript" src="<sys:context/>/resource/js/artDialogLogin.js"></script>-->
</body>
</html>
