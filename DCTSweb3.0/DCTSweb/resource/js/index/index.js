		...
		success:function(data){//回传函数
			...
			var openHtml = '';
   				...
   				for(var i = 0; i < dataObj.list.length; i++) {
   					...
   					openHtml += '<tr>'+
	              					'<td title="'+dataObj.list[i].cargoName+'">'+dataObj.list[i].cargoName+'</td>'+
	...
	$.ajax({
		...
		success:function(data){//回传函数
			...
			var countTradeHtml = '<li class="bno">待确认订单<br /><a href="'+jcontextPath+'/querySuccessCloseTransactionInfo?transactionInfoDomain.tradeStart=1&transactionInfoDomain.menuAId="><span class="dius">'+dataObj.waitingDriverTrade+'</span></a></li>'+
								  ...
								  '<li>待评价订单<br /><a href="'+jcontextPath+'/queryTransactionInfo?transactionInfoDomain.menuAId=a_id_5&transactionInfoDomain.tradeStart=8&transactionInfoDomain.cargoId="><span class="dius">'+dataObj.successNoAssessTrade+'</span></a></li>';
...
function todayDynamicInfo(){
	$.ajax({
		...
		type:'post',	
		dataType:'json', 
		//data:{province:province,city:city}, //参数     tradeList driverList	               
		success:function(data){//回传函数
			var tradeListObj=data.content.tradeList;
			var driverListObj=data.content.driverList; 
			var dataHtml="";
			var countDHtml ="";
			countDHtml+='<li style="color:#FFA042;">哈尔滨百利与黑A12****达成交易</li>';
				for(var i=0;i<tradeListObj.length;i++){
					carNumbers=tradeListObj[i].carNumber.substr(0,4);
					dataHtml+='<li style="color:#FFA042;">'+tradeListObj[i].companyName+'与'+carNumbers+'****达成交易</li>';
				} 
				for(var i=0;i<driverListObj.length;i++){
					var startCode=driverListObj[i].code.substr(0,3);
					var endCode=driverListObj[i].code.substr(driverListObj[i].code.length-4,4);
					dataHtml+='<li style="color:#468cc8;">'+startCode+'****'+endCode+'司机注册成为快到网会员</li>';
				}
				countDHtml+=dataHtml;
