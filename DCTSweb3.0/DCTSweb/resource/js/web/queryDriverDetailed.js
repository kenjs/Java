			...
			driverCarHtml += '<h3><i>&nbsp;</i>推荐车源</h3>';
	   		for (var i=0;i<dataObj.length;i++){
	   			...
	   			driverCarHtml += '<ul>'+
	   								...
	   								'<li class="seef"><a target="_blank" href="'+jcontextPath +'/openDriverDetailed.action?driverId='+dataObj[i].id+'">查看详情</a></li>'+
	   							 '</ul>';
	   		}
...
function driverTransactionInfoById(curPage) {
	...
	$.ajax({
		type: "POST",
	   	...
	   	dataType:"json",
	   	data:{driverId:driverId,curPage:curPage,pageSize:8}, //参数     	
		success:function(data){//回传函数
	   		if(data.result == 1) {
				...
				window.open(url);
	   			...
	   			for(var i = 0;i<dataObj.list.length;i++) {
	   				driverTransactionInfoHtml +=  '<tr>'+
				  	              					...
				  	              					'<td><span class="icon">&nbsp;</span>'+dataObj.list[i].companyName+'</td>'+
