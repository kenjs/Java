...
//取消订单(我的订单列表和订单详情页,个人中心的首页)您确定要取消订单吗？取消后不能恢复!
 function cancleTranceDialog(id,cargoFlag,tradeStart,userDriverAssessCount,channelId,userId,okFunc){
   ...
   ok: function () {
  	...
  	execDatabaseCancleOrder(url,dataParameter,channelId,userId,okFunc);
	...
	$.ajax({
		...
		success:function(data){//回传函数
			...
			var countTradeHtml = '<li>交易提醒：</li>'+
								  ...
								  '<li><a href="'+jcontextPath+'/queryTransactionInfo?transactionInfoDomain.menuAId=a_id_5&transactionInfoDomain.tradeStart=8&transactionInfoDomain.cargoId=">待评价订单<span class="dius">'+dataObj.successNoAssessTrade+'</span></a></li>';
...
//取消订单与后台交互
function execDatabaseCancleOrder(url,dataParameter,channelId,userId,okFunc){//方法作为参数
	   $.ajax({
					url: url,   
					type:'post',	
					dataType:'json', 
					...
					success:function(data){//回传函数
						if(data.result == 0) {//成功
							baiduPushMessagesToDriverFourth(channelId,userId,okFunc);
...
//取消订单成功刷新原页面后百度云推送消息 20140724 PM 
function baiduPushMessagesToDriverFourth(channelId,userId,okFunc){
	//1.个人中心和我的订单页面取消订单成功后刷新原页面；订单详情页面取消订单后跳到我的订单页面？？（若从个人中心首页点过去的呢）
	okFunc();
	if(channelId==null||channelId==""||userId==null||userId==""){
		return;
	}
		   $.ajax({
						...
						type:'post',	
						dataType:'json', 
						data:{'driverUserInfoDomain.baiduChannelId':channelId,'driverUserInfoDomain.baiduUserId':userId,'type':'3'},      	               
						success:function(data){//回传函数
						}
					});
...
//点击"收货确认"或"到货确认"(即修改订单表中的发货时间和到货时间)
function modifyTimeHandle(id,time,aVal,defalutDeliiver){
 var lableVal="";
 var content="";
 content+='<form id="modifyTimeForm" action="/modifyDeliveryOrArrivalTime" namespace="/" method="post">'+
        '<input type="hidden" id="tradesId" value="'+id+'" name="transactionInfoDomain.id"/>'+
        '<div><b id="timesPrompt" style="color:red"></b></div>';
 if(time=='deliveryTime'){
    lableVal='发货时间<font color="red">*</font> ';
    content+=lableVal+'<input  onfocus=cleanContext("timesPrompt") name="transactionInfoDomain.'+time+'" value="'+defalutDeliiver+'" id="'+time+'Id" readonly="readonly" onClick="WdatePicker()" class="intu"/>';
 }else{
   lableVal='到货时间<font color="red">*</font> ';
   content+=lableVal+'<input  onfocus=cleanContext("timesPrompt") name="transactionInfoDomain.'+time+'" value="" id="'+time+'Id" readonly="readonly" onClick="WdatePicker()" class="intu"/>';
   }
  content+= '</form>';
  art.dialog({
  follow: document.getElementById(aVal+id),
  content: content,
  ok: function () {
    var times=$("#"+time+"Id").val();
  	if(times==""){
  		 $("#timesPrompt").html(lableVal+"必填");
  		 return false;
  	}
    var formVal=$("#modifyTimeForm").serialize();
    execDatabaseInteractionHandle(jcontextPath+'/modifyDeliveryOrArrivalTime',formVal,reload);
  },
  cancel: true
 });
}
...
//20140818 订车和换车,输入司机号码（ 导入的订单没有司机号码及运输跟踪中的订单可换车）
function confirmDrivers(tradeId,tradeOrder){
var content='<div style="color:red;margin-left: 75px;" id="promptInfo"></div>'+
'<label>承运司机号码:</label>'+
'<input id="codeId" name="transactionInfoDomain.code" onfocus=cleanContext("promptInfo") value="${transactionInfoDomain.code }" type="text" class="int m3" />';

 art.dialog({
 follow: document.getElementById(tradeId+'cd'),
 content: content,
 ok: function () {
  var code=$("#codeId").val();
	if(code==""){
		 $("#promptInfo").html("请填写司机号码");
		 return false;
	}
  var dataParameter={'transactionInfoDomain.code':code,'transactionInfoDomain.id':tradeId,'transactionInfoDomain.tradeOrder':tradeOrder};
  execDatabaseInteractionHandle(jcontextPath+'/modifyTransactionDriver',dataParameter,reload);
},
cancel: true
});
}